...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Account created! Welcome to LinguaSense!')
      navigate('/dashboard')
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#030712] flex items-center justify-center p-4 relative overflow-hidden">
      <div className="absolute w-96 h-96 bg-purple-600/20 rounded-full blur-3xl top-0 right-0 pointer-events-none" />
      <div className="absolute w-80 h-80 bg-cyan-600/20 rounded-full blur-3xl bottom-0 left-0 pointer-events-none" />
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">Start analyzing multilingual text for free</p>
        </div>

        <div className="glass-card rounded-2xl p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            {[
              { key: 'name', label: 'Full Name', icon: User, type: 'text', placeholder: 'Ahmed Khan' },
